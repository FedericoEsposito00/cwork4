...
bool service_callback(cwork4::getRelativePose::Request &req, cwork4::getRelativePose::Response &res) {
    ...
    try {
        listener.waitForTransform(req.frame.data, req.ref_frame.data, ros::Time(0), ros::Duration(3.0));
        listener.lookupTransform(req.frame.data, req.ref_frame.data, ros::Time(0), transform);
        working_fine = true;    
